                ...
                if (!archiveEntry.isDirectory()) {
                    ...
                    if (parts.length > (archiveHasManySubmissions ? 1 : 0)) {
                        ...
                        for (String part : parts) {
                            EntityArchive nextArchive = Objects.requireNonNull(curArchive).getChildren_() == null ? null : curArchive.getChildren_().stream().filter(x -> x.getName().equals(part)).findAny().orElse(null);
