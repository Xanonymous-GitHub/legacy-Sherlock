...
public class NGramDetector extends PairwiseDetector<NGramDetectorWorker> {
    ...
    /**
     * Sets metadata for the detector, along with providing the API with pointers to the Worker and the Preprocessing Strategy
     */
    ...
    public float compare(ArrayList<Ngram> string1, ArrayList<Ngram> string2) {
        ...
        ArrayList<String> string1List = new ArrayList<>();
        ArrayList<String> string2List = new ArrayList<>();
        ...
        // for each N-gram in the checklist count the number that don't match (matches counted in last loop)
    ...
    public void matchFound(ArrayList<Ngram> reference, ArrayList<Ngram> check, Ngram head, float last_peak, int since_last_peak, ISourceFile file1, ISourceFile file2) {
        ...
        if (reference.size() >= minimum_window) {
            ...
            NgramMatch temp =
                    new NgramMatch(reference.getFirst().getLineNumber(), reference.getLast().getLineNumber(), check.getFirst().getLineNumber(), check.getLast().getLineNumber(),
            ...
            res.put(temp, reference.getFirst().getLineNumber(), reference.getLast().getLineNumber(), check.getFirst().getLineNumber(), check.getLast().getLineNumber());
        }
    ...
    private void loadNgramList(ArrayList<Ngram> storage_list, ArrayList<IndexedString> file) {
        ...
        for (IndexedString lineC : file) {
            // acquire line
            line = new StringBuilder(lineC.getValue());
            ...
            if (line.length() < ngram_size) {
                ...
                line.append(" ".repeat(Math.max(0, ngram_size - line.length() + 1)));
            }
            // acquire line number
    ...
    public class NGramDetectorWorker extends PairwiseDetectorWorker<NGramRawResult> {
        ...
        public void execute() {
            ...
            HashMap<String, Ngram> storage_map = new HashMap<>();
            loadNgramMap(storage_map, linesF1);
            // generate the N-grams for file 2 and load them into a list
            ArrayList<Ngram> storage_list = new ArrayList<>();
            loadNgramList(storage_list, linesF2);

            // start of file check
            Ngram substrObj;
            ArrayList<Ngram> reference = new ArrayList<>();
            ArrayList<Ngram> check = new ArrayList<>();
            Ngram head = null;
            ...
            for (int i = 0; i < storage_list.size(); i++) {
                ...
                if (storage_map.containsKey(ngram_string + ngram_id) || !reference.isEmpty()) {
                    ...
                    if (reference.size() == minimum_window && sim_val < threshold) {
                        ...
                        if (storage_map.containsKey(reference.getFirst().getNgram() + (ngram_id + 1))) {
